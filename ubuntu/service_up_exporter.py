...
def generate():
    for s in services:
        try:
            ...
            g.labels(host,s).set(0)
    g.labels(s,"dropped_packets").set(5)
    g.labels(s,"tx_packets").set(5)
    g.labels(s,"rx_packets").set(5)
    g.labels(host,"up_time").set(uptime1())
    g.labels(s,"dropped_packets").set(5)
    g.labels("VenkiteshTest3","Appdata3").set(8)
